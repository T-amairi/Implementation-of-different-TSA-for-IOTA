...
void TxActorModule::handleMessage(cMessage * msg)
{
    ...
    if( msg->isSelfMessage() ){
        ...
        { //POW_TIMER
            ...
            else if(strcmp( par( "tipSelectionMethod" ), "WPWALKTrack" ) == 0) //WPWALKTrack
            {
               t_txApproved chosenTips = self.WhitePaperTSATrack( par( "walkAlphaValue" ), actorTipView, tipTime, par("WalkDistance"),par("NumberofWalkers"),TxActorId);
               self.attach(actorTipView,tipTime,chosenTips);
            }

            else if(strcmp( par( "tipSelectionMethod" ), "WPWALK" ) == 0) //WPWALK
            {
               t_txApproved chosenTips = self.WhitePaperTSA( par( "Alpha" ), actorTipView, tipTime, par("WalkDistance"),par("NumberofWalkers"));
               self.attach(actorTipView,tipTime,chosenTips);
             }

            else if(strcmp( par( "tipSelectionMethod" ), "GIOTA" ) == 0) //GIOTA
            {
               t_txApproved chosenTips = self.GIOTA( par( "Alpha" ), actorTipView, tipTime, par("WalkDistance"),par("NumberofWalkers"));
               self.attach(actorTipView,tipTime,chosenTips);
            }

            else if(strcmp( par( "tipSelectionMethod" ), "EIOTA" ) == 0) //EIOTA
            {
               t_txApproved chosenTips = self.EIOTA( par( "p1" ), par( "p2" ),actorTipView, tipTime, par("WalkDistance"),par("NumberofWalkers"));
               self.attach(actorTipView,tipTime,chosenTips);
            }
...
{
    //tn.printTangle(tn.allTx,tn.giveGenBlock());
    ...
    else if( msg->getKind() == ATTACH_CONFIRM )
    {
        ...
        if( justAttached->TxNumber >= txLimit )
        {
            ...
            tn.printTangle(tn.allTx,tn.giveGenBlock());
            tn.printTipsLeft(static_cast<int>(tn.giveTips().size()));
