...
class Tangle
{
    ...
    public:
        ...
        //tracking
        void printTangle(std::vector<t_ptrTx> allTx, const t_ptrTx& Gen);
...

{
    ...
    public:
        ...
        //TSA based on the original WhitePaper of Serguei Popov with Tracking
        t_ptrTx getWalkStartWPTrack(std::map<int, t_ptrTx>& tips, int backTrackDist, std::string TxActorId);
        t_ptrTx WPWalkTipSelectionTrack(t_ptrTx start, double alphaVal, std::map<int, t_ptrTx>& tips, omnetpp::simtime_t timeStamp, int &walk_time, std::string TxActorId);
        t_txApproved WhitePaperTSATrack(double alphaVal, std::map<int, t_ptrTx>& tips, omnetpp::simtime_t timeStamp, int W, int N, std::string TxActorId);

        //TSA based on the original WhitePaper of Serguei Popov without Tracking
        t_ptrTx WPWalkTipSelection(t_ptrTx start, double alphaVal, std::map<int, t_ptrTx>& tips, omnetpp::simtime_t timeStamp, int &walk_time);
        t_txApproved WhitePaperTSA(double alphaVal, std::map<int, t_ptrTx>& tips, omnetpp::simtime_t timeStamp, int W, int N);

        //GIOTA
        t_txApproved GIOTA(double alphaVal, std::map<int, t_ptrTx>& tips, omnetpp::simtime_t timeStamp, int W, int N);

        //EIOTA
        t_txApproved EIOTA(double p1, double p2, std::map<int, t_ptrTx>& tips, omnetpp::simtime_t timeStamp, int W, int N);
